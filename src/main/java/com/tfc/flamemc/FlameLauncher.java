	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				for (String s:mods) {
					File fi1=new File(s);
					IFlameMod mod = ((IFlameMod)loader.loadClass("entries."+fi1.getName().replace(".zip","").replace(".jar","")+".Main").newInstance());
					mod.preinit(args);
					mod.init(args);
					mod.postinit(args);
				}
...
//			JarClassLoader loader = new JarClassLoader(new File(dir+"\\versions\\1.15.2-flame\\1.15.2-flame.jar").toURL());
		} catch (Throwable err) {
			FlameConfig.logError(err);
		}
